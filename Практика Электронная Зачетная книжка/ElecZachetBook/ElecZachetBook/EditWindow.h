...
#pragma endregion
	...
	private: System::Void EditButton_Click(System::Object^ sender, System::EventArgs^ e)
	{
		...
		else
		{
			...
			if (dateOfExam == "  .  .")
			{
				dateOfExam = "дата не задана";
			}
			disciplineName = disciplineName->Replace(";", "")->Replace(" ", "");
			themes = themes->Replace(";", "")->Replace(" ", "");
		...
		EditDateOfExamTb->Text = editDate;
		   ...
		   String^ errorMessageSave()
		   {
			   ...
			   else
			   {
				   ...
				   if (EditDateOfExamTb->Text == "  .  .") {}
				   else if (!EditDateOfExamTb->MaskFull)
				   {
					   error += k.ToString() + ") " + "Дата должна быть заполнена полностью или не заполнена вовсе! (пример: 12.12.2002)";
				   }
			   }
			   return error;
		   }
